...
from .postprocessing import seg_det_postprocess
...
class SegDetFPNHead(nn.Module):
    ...
    def forward(self, features, gt_segmap=None, gt_contour=None, gt_objmask=None, gt_classes=None):
        ...
        if self.training:
            ...
            pos_weight_s = torch.ones([self.num_classes])*2  # 10
...
#             )
            ...
            pos_weight_c = torch.ones([self.num_classes])*5  # 20
            ...
            if len(loss_emb1d):
                losses["embedding"] = loss_emb1d.mean() * self.loss_weight * 0.01
            else:
            ...
            #segmap = F.sigmoid(segmap)  # use sigmoid on logits ONLY
