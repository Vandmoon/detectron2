...
def blend(image1, image2, factor):
  if autoaug_debug: print("start blend")
...
def cutout(image, pad_size, replace=0):
  if autoaug_debug: print("start cutout")
...
def solarize(image, threshold=128):
  if autoaug_debug: print("start solarize")
...
def solarize_add(image, addition=0, threshold=128):
  if autoaug_debug: print("start solarize_add")
...
def color(image, factor):
  if autoaug_debug: print("start color")
...
def contrast(image, factor):
  if autoaug_debug: print("start contrast")
...
def brightness(image, factor):
  if autoaug_debug: print("start brightness")
...
def posterize(image, bits):
  if autoaug_debug: print("start posterize")
...
def rotate(image, degrees, replace):
  if autoaug_debug: print("start rotate")
...
def random_shift_bbox(image, bbox, pixel_scaling, replace,
  ...
  if autoaug_debug: print("start random_shift_bbox")
...
def _clip_bbox(min_y, min_x, max_y, max_x):
  if autoaug_debug: print("start _clip_bbox")
...
def _check_bbox_area(min_y, min_x, max_y, max_x, delta=0.05):
  if autoaug_debug: print("start _check_bbox_area")
...
def _scale_bbox_only_op_probability(prob):
  if autoaug_debug: print("start _scale_bbox_only_op_probability")
...
def _apply_bbox_augmentation(image, bbox, augmentation_func, *args):
  if autoaug_debug: print("start _apply_bbox_augmentation")
...
def _concat_bbox(bbox, bboxes):
  if autoaug_debug: print("start _concat_bbox")
...
def _apply_bbox_augmentation_wrapper(image, bbox, new_bboxes, prob,
                                     ...
                                     *args):
  if autoaug_debug: print("start _apply_bbox_augmentation_wrapper")
...
def _apply_multi_bbox_augmentation(image, bboxes, prob, aug_func,
  ...
  if autoaug_debug: print("start _apply_multi_bbox_augmentation")
...
def _apply_multi_bbox_augmentation_wrapper(image, bboxes, prob, aug_func,
                                           func_changes_bbox, *args):
  if autoaug_debug: print("start _apply_multi_bbox_augmentation_wrapper")
...
def rotate_only_bboxes(image, bboxes, prob, degrees, replace):
  if autoaug_debug: print("start rotate_only_bboxes")
...
def shear_x_only_bboxes(image, bboxes, prob, level, replace):
  if autoaug_debug: print("start shear_x_only_bboxes")
...
def shear_y_only_bboxes(image, bboxes, prob, level, replace):
  if autoaug_debug: print("start shear_y_only_bboxes")
...
def translate_x_only_bboxes(image, bboxes, prob, pixels, replace):
  if autoaug_debug: print("start translate_x_only_bboxes")
...
def translate_y_only_bboxes(image, bboxes, prob, pixels, replace):
  if autoaug_debug: print("start translate_y_only_bboxes")
...
def flip_only_bboxes(image, bboxes, prob):
  if autoaug_debug: print("start flip_only_bboxes")
...
def solarize_only_bboxes(image, bboxes, prob, threshold):
  if autoaug_debug: print("start solarize_only_bboxes")
...
def equalize_only_bboxes(image, bboxes, prob):
  if autoaug_debug: print("start equalize_only_bboxes")
...
def cutout_only_bboxes(image, bboxes, prob, pad_size, replace):
  if autoaug_debug: print("start cutout_only_bboxes")
...
def _rotate_bbox(bbox, image_height, image_width, degrees):
  if autoaug_debug: print("start _rotate_bbox")
...
def rotate_with_bboxes(image, bboxes, degrees, replace):
  if autoaug_debug: print("start rotate_with_bboxes")
...
def translate_x(image, pixels, replace):
  if autoaug_debug: print("start translate_x")
...
def translate_y(image, pixels, replace):
  if autoaug_debug: print("start translate_y")
...
def _shift_bbox(bbox, image_height, image_width, pixels, shift_horizontal):
  if autoaug_debug: print("start _shift_bbox")
...
def translate_bbox(image, bboxes, pixels, replace, shift_horizontal):
  if autoaug_debug: print("start translate_bbox")
...
def shear_x(image, level, replace):
  if autoaug_debug: print("start shear_x")
...
def shear_y(image, level, replace):
  if autoaug_debug: print("start shear_y")
...
def _shear_bbox(bbox, image_height, image_width, level, shear_horizontal):
  if autoaug_debug: print("start _shear_bbox")
...
def shear_with_bboxes(image, bboxes, level, replace, shear_horizontal):
  if autoaug_debug: print("start shear_with_bboxes")
...
def autocontrast(image):
  if autoaug_debug: print("start autocontrast")
...
def sharpness(image, factor):
  if autoaug_debug: print("start sharpness")
...
def equalize(image):
  if autoaug_debug: print("start equalize")
...
def wrap(image):
  if autoaug_debug: print("start wrap")
...
def unwrap(image, replace):
  if autoaug_debug: print("start unwrap")
...
def _cutout_inside_bbox(image, bbox, pad_fraction):
  if autoaug_debug: print("start _cutout_inside_bbox")
...
def bbox_cutout(image, bboxes, pad_fraction, replace_with_mean):
  if autoaug_debug: print("start bbox_cutout")
...
def autoaugdet(image, bboxes, type):
    for box in bboxes:
        box[2] = box[0] + box[2]
        box[3] = box[1] + box[3]
    h,w,c = image.shape
#     print("=====Original image size: ", h, w, c)
    ...
    bboxes[:, 3] = temp
    ####################################################################################################################
    image, bboxes = distort_image_with_autoaugment(tf.convert_to_tensor(image), tf.to_float(tf.convert_to_tensor(bboxes)), type)
    image = image.numpy()
    bboxes = bboxes.numpy()
    ####################################################################################################################
#     h,w,c = image.shape
#     print("=====Augmented image size: ", h, w, c)
    temp = bboxes[:, 0] * h
    bboxes[:, 0] = bboxes[:, 1] * w
    bboxes[:, 1] = temp
    temp = bboxes[:, 2] * h
    ...
    for i in range(bboxes.shape[0]):
        if bboxes[i, 2]-bboxes[i, 0] <= 5 or bboxes[i, 3]-bboxes[i, 1] <= 5:
            raise Exception("discard this augment")
        out_bboxes.append([bboxes[i, 0], bboxes[i, 1], bboxes[i, 2]-bboxes[i, 0], bboxes[i, 3]-bboxes[i, 1]])
